...
namespace FITCCRS2.Services.Auth
{
    public class IdentityTokenClaimService : ITokenClaimService
    {
        ...
        public async Task<string> GetTokenAsync(string userName)
        {
            ...
            //var key = Encoding.ASCII.GetBytes("AuthKeyOfDoomThatMustBeAMinimumNumberOfBytes");
            ...
            var tokenHandler = new JwtSecurityTokenHandler();
            var key = Encoding.ASCII.GetBytes("SecretKeyOfDoomThatMustBeAMinimumNumberOfByte");
            var user = await _userManager.FindByNameAsync(userName);
            var roles = await _userManager.GetRolesAsync(user);
            var claims = new List<Claim> { new(ClaimTypes.Name, userName) };
